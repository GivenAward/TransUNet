...
from tensorflow.keras.layers import Layer, Dense, Lambda, Dropout, LayerNormalization
...
class MultiHeadSelfAttention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.query_dense = Dense(
            hidden_size, name=f"query")
        self.key_dense = Dense(
            hidden_size, name=f"key")
        self.value_dense = Dense(
            hidden_size, name=f"value")
        self.combine_heads = Dense(
            hidden_size, name=f"out")
...
class TransformerBlock(Layer):
    ...
    def __init__(self, *args, n_heads, mlp_dim, dropout=0.1, trainable=True, **kwargs):
    ...
    def build(self, input_shape):
        ...
        self.layernorm1 = LayerNormalization(
            epsilon=1e-6, name=f"LayerNorm_0"
        )
        self.layernorm2 = LayerNormalization(
            epsilon=1e-6, name=f"LayerNorm_2"
        )
